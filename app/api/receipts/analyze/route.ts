...
export async function POST(req: NextRequest) {
    try {
        ...
        // Fetch known SKU mappings for this household to improve matching accuracy
            ...
            try {
                ...
                if (supabaseUrl && supabaseServiceKey) {
                    ...
                    if (skuData && skuData.length > 0) {
                        ...
                        skuMappingsSection = `
      KNOWN SKU MAPPINGS (use these for high-confidence matching when you see these SKUs):
      ${storeEntries}

      If you extract a SKU from the receipt that matches one above, use the mapped item name for 'ai_match'.
`;
